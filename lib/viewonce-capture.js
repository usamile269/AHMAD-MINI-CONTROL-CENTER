...
async function handleAntiViewOnce(conn, mek) {
    try {
        ...
        const botNumber = conn.__miniBotNumber || require('@whiskeysockets/baileys').jidNormalizedUser(conn.user.id).split('@')[0];
        ...
        const selfJid = conn.__miniBotJid || `${botNumber}@s.whatsapp.net`;
