...
async function ensureYtDlp() {
    ...
    if (needsFreshBinary) {
        ...
        if (process.platform === 'win32') {
            ...
            fs.writeFileSync(YTDLP_BIN, Buffer.from(dlRes.data));
...
async function raceQuickApis(videoUrl, attempt = 1) {
    try {
        const apiUrl = `https://adeel-xtech-apis.vercel.app/api/ytmp4?url=${encodeURIComponent(videoUrl)}`;
        const res = await axios.get(apiUrl, { ...AXIOS_DEFAULTS, timeout: 7000 });
        const link = res.data?.status && res.data?.result?.video_download;
        if (!link) throw new Error('AdeelXtech: no usable result');
        ...
        if (buf.length < 15000) throw new Error('AdeelXtech: file too small');
        return buf;
    } catch (e) {
        const isHttpErrorResponse = e.response && e.response.status;
        if (attempt < 2 && !isHttpErrorResponse) {
            await new Promise(r => setTimeout(r, 800));
            return raceQuickApis(videoUrl, attempt + 1);
        }
        throw e;
    }
}
...
// 🚨 BUG FIX (Bunty: ".play phir se slow ho gaya"): unlike raceQuickApis
// (video), this had NO retry — a single transient blip on AdeelXtech's
// ytmp3 endpoint (cold start, brief rate-limit) failed the quick path
// instantly and sent EVERY request down the slow yt-dlp+ffmpeg route,
// which is exactly the "was fast, now slow again" pattern. Same one-retry
// treatment as the video path now.
// 🚨 SPEED FIX (Bunty's own logs confirmed AdeelXtech's ytmp3 endpoint is
// currently DEAD — consistent HTTP 500 on every call, not a timeout/blip):
// retrying a definite 500 response gets the same 500 again, so it's pure
// wasted time/CPU on an already memory-constrained host. Only retry on
// network-level failures (timeout, DNS, connection reset) — a clean HTTP
// error response fails straight through to the next method instead.
async function getQuickAudioBuffer(videoUrl, attempt = 1) {
    try {
        const apiUrl = `https://adeel-xtech-apis.vercel.app/api/ytmp3?url=${encodeURIComponent(videoUrl)}`;
        const { data } = await axios.get(apiUrl, { ...AXIOS_DEFAULTS, timeout: 7000 });
        const r = data?.result || {};
        const link = (data?.status && (r.audio_download || r.download_url || r.audio || r.url)) || null;
        if (!link) throw new Error('AdeelXtech ytmp3: no usable result');
        ...
        if (!isLikelyAudio(buf)) throw new Error('AdeelXtech ytmp3: response not valid audio');
        return buf;
    } catch (e) {
        const isHttpErrorResponse = e.response && e.response.status;
        if (attempt < 2 && !isHttpErrorResponse) {
            await new Promise(r => setTimeout(r, 800));
            return getQuickAudioBuffer(videoUrl, attempt + 1);
        }
        throw e;
    }
}
...
async function dlAudio(videoUrl, outPath) {
    ...
    // full video.
    const wrap = await ensureYtDlp();
    const fastArgs = [videoUrl, '-f', 'bestaudio/best', '-x', '--audio-format', 'mp3', '--audio-quality', '0',
...
async function dlVideo(videoUrl, outPath) {
    try {
        const buf = await raceQuickApis(videoUrl);
        fs.writeFileSync(outPath, buf);
        return;
    } catch (e) {
        const msgs = (e.errors || [e]).map(x => x.message).join(' | ');
        console.log('[YTMP4] both quick APIs failed, falling back to yt-dlp:', msgs);
    }
...
async (conn, mek, m, { reply, args, from, q }) => {
    ...
    try {
        ...
        try {
            await new Promise((resolve, reject) => {
                ffmpeg(outPath).audioCodec('libopus').audioBitrate('96k').audioChannels(2).format('ogg')
