...
// 🚀 GLOBAL HTTP SPEED: reuse TCP/TLS connections across all axios calls.
// This is initialized before ./main loads the plugin tree, so every plugin
// shares the same keep-alive agents instead of opening a new connection for
// every API request.
const http = require('http');
const https = require('https');
const axios = require('axios');
const keepAliveHttp = new http.Agent({ keepAlive: true, maxSockets: 100, maxFreeSockets: 20, timeout: 60000 });
const keepAliveHttps = new https.Agent({ keepAlive: true, maxSockets: 100, maxFreeSockets: 20, timeout: 60000 });
axios.defaults.httpAgent = keepAliveHttp;
axios.defaults.httpsAgent = keepAliveHttps;
axios.defaults.headers.common.Connection = 'keep-alive';

const { fork } = require('child_process');
const path = require('path');
const { setInternalWorker } = require('./lib/ipc-queue');
...
app.get('/health', (req, res) => {
    res.set('X-Bot-Build', process.env.RAILWAY_GIT_COMMIT_SHA || process.env.GIT_COMMIT_SHA || 'unknown');
    res.set('X-AI-Fallback', 'bj-devs+pollinations');
...
// 🏗️ AUTO-WORKER (Ultimate Auto-Pilot Mode): 
// We ALWAYS spawn an internal background worker child process.
// If REDIS_URL is present, it uses BullMQ (Distributed).
// If NOT, it uses IPC Bridge (Internal Multi-Threading).
// This ensures "World-Class" speed even without any Railway configuration.
const workerPath = path.join(__dirname, 'worker.js');
console.log('[AUTO-WORKER] Spawning background processor for heavy tasks...');
const workerProcess = fork(workerPath, [], {
    env: { ...process.env, WORKER_CONCURRENCY: process.env.WORKER_CONCURRENCY || '2' },
    stdio: 'inherit'
});
setInternalWorker(workerProcess);

workerProcess.on('error', (err) => console.error('[AUTO-WORKER] process error:', err.message));
workerProcess.on('exit', (code) => {
    if (code !== 0) console.warn(`[AUTO-WORKER] background processor exited with code ${code}.`);
});
